...
export function PortfolioTemplate({
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-[#e5e5e5] selection:bg-[#d4a373] selection:text-black">
      {/* Edit Button */}
      {isPreview && (
        <motion.div
      ...
      {/* Navigation */}
      <nav className="fixed left-0 right-0 top-0 z-40 flex items-center justify-between px-6 py-6 backdrop-blur-sm md:px-12 md:py-8">
      ...
      {/* Hero Section */}
      <section className="flex min-h-[85vh] flex-col justify-end px-6 pb-20 pt-32 md:min-h-[90vh] md:px-12 md:pb-24">
        <div className="max-w-[90rem]">
          <motion.p
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8 }}
            className="mb-6 font-mono text-xs uppercase tracking-widest text-[#d4a373] md:mb-8 md:text-sm"
          >
            {title}
          </motion.p>
          <motion.h1
            ...
            className="font-instrument text-[14vw] leading-[0.9] text-[#e5e5e5] md:text-[9vw] lg:text-[8vw]"
          >
          ...
          {tagline && (
            <motion.div
              ...
              className="mt-8 max-w-xl md:mt-12 md:max-w-2xl"
            >
              <p className="font-instrument text-xl italic leading-relaxed text-neutral-400 md:text-3xl">
                {tagline}
              </p>
            </motion.div>
          )}
      ...
      <section id="about" className="px-6 py-16 md:px-12 md:py-24">
        <div className="flex flex-col gap-8 md:flex-row md:items-start md:gap-24">
          ...
          <div className="md:w-3/4 max-w-4xl">
            {bio && (
              <p className="mb-12 text-lg leading-relaxed text-neutral-300 md:text-2xl font-light">
                {bio}
              </p>
            )}
      ...
      <section id="projects" className="px-6 py-16 md:px-12 md:py-24">
        ...
        <div className="grid gap-x-12 gap-y-16 md:grid-cols-2 md:gap-y-24">
          {projects.map((project, index) => (
            ...
            >
              <div className="aspect-[4/3] w-full overflow-hidden bg-[#111] ring-1 ring-white/5">
                <div className="h-full w-full bg-neutral-900 transition-transform duration-700 group-hover:scale-105" />
              </div>

              <div className="flex items-start justify-between">
                <div>
                  <h3 className="mb-2 font-instrument text-2xl text-white transition-all duration-300 group-hover:italic md:text-3xl">
                    {project.title}
                  </h3>
                ...
                {project.live && (
                  <a
                    ...
                    className="flex h-10 w-10 items-center justify-center rounded-full border border-white/10 transition-colors hover:bg-white hover:text-black"
                  >
      ...
      {experience.length > 0 && (
        <section className="px-6 py-16 md:px-12 md:py-24">
          <div className="mb-12 border-b border-white/10 pb-6 md:mb-16">
            <h2 className="font-mono text-xs uppercase tracking-widest text-neutral-500 md:text-sm">
              (03) Experience
            </h2>
          </div>
          ...
          <div className="max-w-5xl">
            {experience.map((exp) => (
              <div
                key={exp.id}
                className="group border-b border-white/5 py-8 transition-colors hover:border-white/20 md:py-12"
              >
                <div className="flex flex-col gap-2 md:flex-row md:items-baseline md:justify-between">
                  <h3 className="font-instrument text-3xl text-neutral-300 transition-colors group-hover:text-white md:text-4xl">
                    {exp.company}
                  </h3>
                ...
                {exp.description && (
                  <p className="mt-4 max-w-xl text-sm leading-relaxed text-neutral-500 md:mt-6 md:text-base">
                    {exp.description}
                  </p>
                )}
              </div>
      ...
      {/* Contact Footer */}
      <footer id="contact" className="px-6 py-16 md:px-12 md:py-24">
        <div className="flex min-h-[50vh] flex-col justify-between rounded-[2rem] bg-[#111] p-8 md:p-16">
          ...
          <div className="mt-12 md:mt-0">
            <p className="mb-4 font-mono text-xs text-neutral-500 md:text-sm">
              Have an idea?
            </p>
            <a
              href={`mailto:${socialLinks.email}`}
              className="block font-instrument text-5xl text-white transition-all hover:text-[#d4a373] hover:italic md:text-8xl lg:text-9xl"
            >
              Let&apos;s talk.
            </a>
          </div>

          <div className="mt-12 flex flex-col gap-4 border-t border-white/10 pt-8 md:mt-16 md:flex-row md:items-center md:justify-between">
